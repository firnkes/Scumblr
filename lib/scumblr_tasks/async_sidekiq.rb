...
module ScumblrWorkers
  class AsyncSidekiqWorker
    ...
    def create_event(event, level="Error")
      ...
      event_details = Event.create(action: level, eventable_id: eventable_id, eventable_type: "Task", source: "Task: #{self.class.to_s}", details: details, user_id: @options[:_user].id)
